...
require_once('../resources/OAuth.php');
  ...
  if (!empty($_SESSION['outcome'])) {
    $params['lis_outcome_service_url'] = get_root_url() . '../tool_consumer/test_outcome.php';
  }
  if (!empty($_SESSION['outcomes'])) {
    $params['ext_ims_lis_basic_outcome_url'] = get_root_url() . '../tool_consumer/test_outcome.php';
  }
  if (!empty($_SESSION['membership'])) {
    $params['ext_ims_lis_memberships_url'] = get_root_url() . '../tool_consumer/test_membership.php';
    $params['ext_ims_lis_memberships_id'] = $_SESSION['membership_id'];
  }
  $params['launch_presentation_return_url'] = get_root_url() . '../tool_consumer/test_harness.php';
